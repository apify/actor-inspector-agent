...
def create_code_quality_agent(llm: str, debug: bool = False) -> Agent:
    ...
    return Agent(
        ...
        backstory=(
            ...
            'Maintain high code quality to ensure readability, reliability, '
            'and scalability. Focus on these key principles:\n\n'
...
def create_actor_quality_agent(llm: str, debug: bool = False) -> Agent:
    ...
    return Agent(
        ...
        backstory=(
            ...
            'My tools include the ability to get the README and input schema of an Actor. '
            'I can help by delivering insights on how well the README is defined and '
            'how easy it is for a real user to follow and use the actor for the first time. '
            'Additionally, I will analyze the input properties descriptions to ensure '
            'they are well-defined and make sense.\n'
            '\nActor README guidelines:'
            "In Apify, a readme is more than a developer guide—it's the public Actor "
            'detail page in the Store, serving as a landing page for users. It has four '
            'main functions: **SEO** (use keywords for Google visibility), '
            '**first impression** (convince users to try your Actor), '
            '**extended instructions** (explain complex inputs), and '
            '**support** (offer troubleshooting and help links).\n'
            '\n### Key Elements:\n'
            '1. **Intro & Features**: Briefly explain what the Actor does, its benefits, '
            'and data it extracts (e.g., with a table). Highlight platform perks like '
            'scheduling or proxies.\n'
            '2. **Tutorial**: Provide simple steps or link to a guide for using the Actor.\n'
            '3. **Pricing**: Clarify costs (e.g., per result or compute units), set '
            'expectations, and optimize for cost-related Google searches.\n'
            '4. **Input/Output**: Show input examples (screenshots) and output data '
            '(JSON or tables).\n'
            '5. **Other Actors**: Promote your related tools.\n'
            '6. **FAQ/Support**: Address common questions, bugs, legality, and contact '
            'options.\n'
            '\n### Tips:\n'
            '- Write in **markdown** (H2/H3 headings for SEO and table of contents).\n'
            "- Match the **tone** to your audience's skill level (technical or beginner-"
            'friendly).\n'
            "- Keep the first 25% engaging—most users don't scroll further.\n"
            '- Use **images/videos** (SEO-friendly, clickable) for better engagement '
            'and ranking.\n'
            '\nA good readme balances user needs, Google discoverability, and clear '
            'guidance, making it a powerful tool to attract and retain users. Aim for '
            'at least 300 words.\n'
        ),
        tools=tools,
        verbose=debug,
